...
func HandleLoginPage(ar *osin.AuthorizeRequest, w http.ResponseWriter, r *http.Request) (osin.ResponseData, bool) {
	r.ParseForm()
	if r.Method == "POST" {
		if user, ok := users[r.Form.Get("login")]; ok {
			return user, true
		}
	}
...
func main() {
	...
	storage.SetClient("1234", &osin.DefaultClient{
		Id:          "1234",
		Secret:      "aabbccdd",
		RedirectUri: "http://192.168.1.115:3000/oauth/weixin",
	})
	...
	http.HandleFunc("/authorize", func(w http.ResponseWriter, r *http.Request) {
		...
		if ar := server.HandleAuthorizeRequest(resp, r); ar != nil {
			user, ok := HandleLoginPage(ar, w, r)
			if !ok {
				return
			}
			ar.Authorized = true
	...
	http.HandleFunc("/sns/oauth2/access_token", func(w http.ResponseWriter, r *http.Request) {
		resp := server.NewResponse()
		defer resp.Close()
	...
	http.HandleFunc("/sns/userinfo", func(w http.ResponseWriter, r *http.Request) {
		resp := server.NewResponse()
		defer resp.Close()

		if ir := server.HandleInfoRequest(resp, r); ir != nil {
			server.FinishInfoRequest(resp, r, ir)
			resp.Output = ir.AccessData.UserData.(osin.ResponseData)
		}
		osin.OutputJSON(resp, w, r)
	})
